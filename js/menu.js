...
var ddlevelsmenu = {
  ...
  arrowpointers: {
    downarrow: ["https://esodb.ru/img/menu/arrow-down.gif", 11, 7], //[path_to_down_arrow, arrowwidth, arrowheight]
    rightarrow: ["https://esodb.ru/img/menu/arrow-right.gif", 12, 12], //[path_to_right_arrow, arrowwidth, arrowheight]
  ...
  addpointer: function (target, imgclass, imginfo, BeforeorAfter) {
    ...
    var target_firstEl =
      target.childNodes[target.firstChild.nodeType != 1 ? 1 : 0]; //see if the first child element within A is a SPAN (found in sliding doors technique)
  ...
  addshimmy: function (target) {
    var shim = !window.opera
      ...
      : document.createElement("div"); //Opera 9.24 doesnt seem to support transparent IFRAMEs
    shim.className = "ddiframeshim";
    shim.setAttribute(
      "src",
      location.protocol == "https:" ? this.httpsiframesrc : "about:blank",
    );
  ...
  positionshim: function (header, submenu, dir, scrollX, scrollY) {
    if (header._istoplevel) {
      ...
      var bottomgap =
        scrollY + this.docheight - header._offsets.top - header._dimensions.h;
      if (topgap > 0) {
      ...
      if (bottomgap > 0) {
        ...
        this.shimmy.bottomshim.style.top =
          header._offsets.top + header._dimensions.h + "px";
  ...
  buildmenu: function (
    ...
    if (istoplevel) {
      this.addEvent(
        header,
        function (e) {
          ddlevelsmenu.hidemenu(
            ddlevelsmenu.subuls[this._master][parseInt(this._pos)],
          );
        },
        "click",
      );
      ...
      function (e) {
        ...
        if (!ddlevelsmenu.isContained(this, e)) {
          ...
          var menuleft = this._istoplevel
            ? this._offsets.left + (dir == "sidebar" ? this._dimensions.w : 0)
            : this._dimensions.w;
          ...
          var menutop = this._istoplevel
            ? this._offsets.top + (dir == "sidebar" ? 0 : this._dimensions.h)
            : this.offsetTop;
          if (submenubottomedge - scrollY > ddlevelsmenu.docheight) {
            ...
            if (
              ...
              this._offsets.top +
                (dir == "sidebar" ? this._dimensions.h : 0) -
                scrollY
            ) {
              //move up?
              menutop +=
              ...
              //top of window edge
              menutop +=
                -(this._offsets.top - scrollY) +
                (this._istoplevel && dir == "topbar" ? -this._dimensions.h : 0);
            }
          }
          submenu.style.top = menutop + "px";
          if (
            ddlevelsmenu.enableshim &&
      ...
      function (e) {
        ...
        if (this._istoplevel) {
          if (
            ...
            !ddlevelsmenu.isContained(submenu, e)
          )
  ...
  showmenu: function (header, submenu, dir) {
    if (this.effects.enableswipe || this.effects.enablefade) {
      if (this.effects.enableswipe) {
        var endpoint =
          header._istoplevel && dir == "topbar"
            ? header._dimensions.submenuh
    ...
    if (elapsed < this.effects.duration) {
      if (this.effects.enableswipe) {
        if (submenu._curanimatedegree == 0) {
          //reset either width or height of sub menu to "auto" when animation begins
          submenu.style[
            header._istoplevel && dir == "topbar" ? "width" : "height"
          ] = "auto";
        }
        submenu.style[
          header._istoplevel && dir == "topbar" ? "height" : "width"
        ] = submenu._curanimatedegree * endpoint + "px";
      }
      if (this.effects.enablefade) {
      ...
      if (this.enableshim && submenu.FFscrollInfo)
        //if this is FF browser (meaning shim hasn't been applied yet
        this.positionshim(
          header,
          submenu,
          dir,
          submenu.FFscrollInfo.x,
          submenu.FFscrollInfo.y,
        );
    }
    submenu._curanimatedegree =
      (1 - Math.cos((elapsed / this.effects.duration) * Math.PI)) / 2;
  },

  hidemenu: function (submenu) {
    if (typeof submenu._pos != "undefined") {
      //if submenu is outermost UL drop down menu
      this.css(
        this.topitems[submenu._master][parseInt(submenu._pos)],
        "selected",
        "remove",
      );
  ...
  domready: function (functionref) {
    //based on code from the jQuery library
    if (dd_domreadycheck) {
      functionref();
      return;
    }
  ...
  init: function (mainmenuid, dir) {
    ...
    for (var i = 0; i < alllinks.length; i++) {
      if (alllinks[i].getAttribute("rel")) {
        ...
        var arrowclass =
          dir == "sidebar" ? "rightarrowpointer" : "downarrowpointer";
        var arrowpointer =
          dir == "sidebar"
            ? this.arrowpointers.rightarrow
        ...
        this.addEvent(
          dropul,
          function (e) {
            //hide menu if mouse moves out of main UL element into open space
            if (
              !ddlevelsmenu.isContained(this, e) &&
              !ddlevelsmenu.isContained(
